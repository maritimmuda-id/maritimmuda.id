        <!-- start page title section -->
        <section class="wow fadeIn parallax" data-stellar-background-ratio="0.5" style="background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_comingsoon_bg-polos-small.png');">
            <div class="opacity-small bg-extra-dark-gray"></div>
            <div class="container">
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12 extra-small-screen display-table page-title-large">
                        <div class="display-table-cell vertical-align-middle text-center">
                            <!-- <h1 class="text-white alt-font font-weight-600 letter-spacing-minus-1 margin-10px-bottom">Beasiswa International The Blue Economist Certification Training Course</h1>
                            <span class="text-white opacity6 alt-font no-margin-bottom">Maritim Muda Nusantara</span> -->
                            <img src="<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_text_bg-outline.png">
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- end page title section -->
        <!-- start section -->
        <section class="wow fadeIn animated color-code md-no-background-img" id="section-down" data-color="8" style="padding-bottom: 0">
            <div class="container margin-one-bottom">
                <div class="row">
                    <div class="col-lg-8 col-md-8 col-sm-10 col-xs-12 text-center center-col margin-eight-bottom xs-margin-30px-bottom last-paragraph-no-margin">
                        <img src="<?php echo site_url('assets') ?>/images/arungisamudra/logo/logo-arungisamudra-inamarys.png" alt="" class=" width-60" style="margin-bottom: 50px;">
                        <p><b>Indonesian Maritime Youth Summit (INAMARYS)</b> adalah sebuah pertemuan pemuda bidang kemaritiman tingkat nasional yang akan diselenggarakan secara hybrid sebagai pra-event menuju International Blue Economy Expo & Summit (IBEES). INAMARYS yang bertemakan “Youth and Digital Transformation for Indonesia's Future Blue Economy” ini akan diselenggarakan dalam dua sesi dengan mengundang narasumber yang profesional dan kredibel di bidangnya untuk membahas topik mengenai “Blue Economy & Marine Conservation” dan “Blue Economy & Education”.</p>
                        <br>
                        <br>
                        <br>
                        <div style="padding:20px 100px 20px 100px; background-color: #0071e7; color: white; border-radius: 20px 0 20px 0;">
                            <h6 class="harond-1" style="margin-bottom: 0;"><a style="color: white;" href="<?php echo site_url('/arungisamudra/exhibitors-inamarys-2024') ?>">List of Digital Exhibitors</a></h6>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- end section -->
        <!-- start section -->
        <div style="background-color: #fad002;">
            <section style="padding-bottom: 0;">
                <div class="container margin-one-bottom">
                    <div class="row">
                        <div class="col-lg-8 col-md-8 col-sm-10 col-xs-12 text-center center-col margin-eight-bottom xs-margin-30px-bottom last-paragraph-no-margin">
                            <h4 class="text-center harond-1" style="color: #039479;">Waktu dan Tempat</h4>
                            <br>
                            <br>
                            <div class="card" style="background-color: white; border: 5px solid #039479; border-color: #039479;">
                                <p style=" padding: 20px;"><b>Hari, Tanggal:</b> Kamis, 20 Juni 2024<br>
                                <b>Pukul:</b> 09.00 WIB - 13.30 WIB<br>
                                <b>Tempat:</b> Auditorium Paisal Kamal, Kampus A STIE Indonesia Jakarta <br>Jalan Kayu Jati Raya No. 11A, Rawamangun, Jakarta Timur</p>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </div>
        <!-- end section -->
        <!-- start section -->
        <div style="background-color: #039479;">
            <section style=" padding-bottom: 0; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-5.png');">
                <div class="container margin-one-bottom">
                    <div class="row">
                        <div class="col-lg-8 col-md-8 col-sm-10 col-xs-12 text-center center-col margin-eight-bottom xs-margin-30px-bottom last-paragraph-no-margin">
                            <h4 class="text-center harond-1" style="color: white;">Fasilitas dan Manfaat</h4>
                            <br>
                            <br>
                            <div class="row">
                                <!-- Row pertama -->
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-four-bottom xs-margin-10px-bottom">
                                    <div class="card" style="border-radius: 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <div style="border-radius: 15px 15px 0 0; background-color: #0071e7; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: right;">
                                            <h6 style="color: white; margin: 0; padding: 10px 0 10px 0;"><span class="harond-1">1</span></h6>
                                        </div>
                                        <p style="color: #030a95; padding: 20px;">Konsumsi (Khusus Peserta Offline)</p>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-four-bottom xs-margin-10px-bottom">
                                    <div class="card" style="border-radius: 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <div style="border-radius: 15px 15px 0 0; background-color: #0071e7; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                            <h6 style="color: white; margin: 0; padding: 10px 0 10px 0;"><span class="harond-1">2</span></h6>
                                        </div>
                                        <p style="color: #030a95; padding: 20px;">E-Certificate</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <!-- Row kedua -->
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-four-bottom xs-margin-10px-bottom">
                                    <div class="card" style="border-radius: 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <div style="border-radius: 15px 15px 0 0; background-color: #0071e7; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: center;">
                                            <h6 style="color: white; margin: 0; padding: 10px 0 10px 0;"><span class="harond-1">3</span></h6>
                                        </div>
                                        <p style="color: #030a95; padding: 20px;">Materi Presentasi</p>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-four-bottom xs-margin-10px-bottom">
                                    <div class="card" style="border-radius: 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <div style="border-radius: 15px 15px 0 0; background-color: #0071e7; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: center;">
                                            <h6 style="color: white; margin: 0; padding: 10px 0 10px 0;"><span class="harond-1">4</span></h6>
                                        </div>
                                        <p style="color: #030a95; padding: 20px;">Grup Telegram Nasional Maritim Muda 38 provinsi dan IKN Nusantara</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <!-- Row ketiga -->
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 margin-four-bottom xs-margin-10px-bottom">
                                    <div class="card" style="border-radius: 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <div style="border-radius: 15px 15px 0 0; background-color: #0071e7; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: center;">
                                            <h6 style="color: white; margin: 0; padding: 10px 0 10px 0;"><span class="harond-1">5</span></h6>
                                        </div>
                                        <p style="color: #030a95; padding: 20px;">Diprioritaskan dalam Program Beasiswa dan Pelatihan di Maritim Muda</p>  
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </section>
        </div>
        <!-- end section -->
        <!-- start section -->
        <div style="background-color: #0071e7; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-ori.png');">
            <section style="padding-bottom: 0;">
                <div class="container margin-one-bottom">
                    <div class="row">
                        <div class="col-lg-8 col-md-8 col-sm-10 col-xs-12 text-center center-col margin-eight-bottom xs-margin-30px-bottom last-paragraph-no-margin">
                            <h4 class="text-center harond-1" style="color: white;">Rundown Kegiatan</h4>
                            <br>
                            <br>
                            <!-- Row 1 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: right;">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">1</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Persiapan</b></p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 2 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: right;">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">2</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Kunjungan ke Sentra Pemberdayaan Pemuda Bidang Kemaritiman Nasional (Prestasi Hub Maritim Muda)</b></p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 3 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">3</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Pembukaan MC, Pembacaan doa, dan menyanyikan lagu Indonesia Raya</b></p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 4 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                            <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: right;">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">4</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Sambutan oleh Ketua Umum Maritim Muda Nusantara, Ketua STIE Indonesia Jakarta, dan Kepala LLDikti Wilayah III</b></p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 5 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">5</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Keynote spech dan pembukaan resmi INAMARYS oleh Asisten Deputi Karakter Pemuda Kemenpora</b></p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 6 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                            <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: right;">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">6</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Special Opening Performance</b></p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 7 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">7</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Grand Launching:</b>
                                            <br>1. Maritim Muda Diving Club
                                            <br>2. Digibluecamp: Learning Management System for Blue Economy Studies and Business Development
                                            <br>3. All New Maritim Muda Hub V.3
                                            <br>4. Website Indonesian Youth Economic Leaders (IYEL)
                                            <br>5. Maritime Technopreneurs Incubator (MARITECHTOR)
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 8 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                            <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png'); background-position: right;">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">8</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Penandatanganan MoU & Dokumentasi</b></p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 9 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">9</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Talkshow Session 1: Blue Economy & Education</b>
                                            <br>1. Dr. I Nyoman Radiarta, Kepala BPPSDM KP KKP dan Penasihat Maritim Muda Nusantara
                                            <br>2. Ibu Martini Mohamad Paham, Deputi Bidang Sumber Daya dan Kelembagaan, Kemenparekraf/Baparekraf
                                            <br>3. Bapak Drs. H. Andri Yansyah, M.H., Kepala Dinas Pemuda dan Olahraga DKI Jakarta
                                            <br>4. Ricky Soeraputra, Co-Founder Kristal Klear Dive Group
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 10 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">10</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Talkshow Session 2: Blue Economy & Marine Conservation</b>
                                            <br>1. Kevin Juan J, Advisor CILT Next Generation
                                            <br>2. Alexandra Maheswari Waskita, Marine Sustainable Financing Specialist, WCS Indonesia
                                            <br>3. Erwin Falufi Irianti, Koordinator Nasional Penjaga Laut
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 11 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">11</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Sharing Session Organisasi Daerah Maritim Muda Nusantara</b>
                                            <br>1. Paka Mutiara (Maritim Muda Bengkulu)
                                            <br>2. Raden Hamdi (Maritim Muda Lampung)
                                            <br>3. Korisnur Cahyono (Maritim Muda Kepulauan Riau)
                                            <br>3. Firdaut Ismail (Maritim Muda Maluku Utara)
                                            <br>3. Zufarnesia (Maritim Muda Sumatera Utara)
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <!-- Row 12 -->
                            <div class="row margin-four-bottom xs-margin-10px-bottom">
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
                                    <div style="border-radius: 15px 15px 0 0; background-color: #f84535; background-image:url('<?php echo site_url('assets/') ?>images/arungisamudra/asset/arungisamudra_bg-patern-4.png');">
                                        <h6 style="color: white; margin: 0; padding: 10px 0;"><span class="harond-1">12</span></h6>
                                    </div>
                                </div>
                                <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12">
                                    <div class="card" style="border-radius: 0 0 20px 20px; background-color: #f7f0d3; border: 5px solid #f7f0d3; border-color: #f7f0d3;">
                                        <p class="text-left xs-text-center" style="color: #030a95; padding: 20px;"><b>Closing</b></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </div>
        <!-- end section -->
        <!-- start section -->
        <section class="wow fadeIn animated color-code md-no-background-img" id="section-down" data-color="8" style="padding-bottom: 0">
            <div class="container margin-one-bottom">
                <div class="row">
                    <div class="col-lg-8 col-md-8 col-sm-10 col-xs-12 text-center center-col margin-eight-bottom xs-margin-30px-bottom last-paragraph-no-margin">
                        <div style="padding:20px; background-color: #f84535; color: white; border-radius: 20px 0 20px 0;">
                            <h6 class="harond-1">Biaya pendaftaran INAMARYS</h6>
                            <div class="row" style="">
                                <div class="container">
                                    <div class="col-md-4 border-right">
                                        <p><u>Offline</u>
                                        <br>Umum: Rp 15.000
                                        <br>Anggota Maritim Muda: Rp 10.000</p>
                                    </div>
                                    <div class="col-md-4">
                                        <p><u>Online</u> (Luar Jabodetabek)
                                        <br>Rp 10.000</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <br>
                        <br>
                        <div style="padding:20px 100px 20px 100px; background-color: #0071e7; color: white; border-radius: 20px 0 20px 0;">
                            <h6 class="harond-1" style="margin-bottom: 0;"><a style="color: white;" href="<?php echo site_url('/arungisamudra/pendaftaran-inamarys-2024') ?>">Daftar sebagai Peserta</a></h6>
                        </div>
                        <br>
                        <hr style="border: 1px solid #0071e7;">
                        <br>
                        <div style="padding:20px 100px 20px 100px; background-color: #0071e7; color: white; border-radius: 20px 0 20px 0;">
                            <h6 class="harond-1" style="margin-bottom: 0;"><a style="color: white;" href="<?php echo site_url('/arungisamudra/pendaftaran-exhibitors-inamarys-2024') ?>">Daftar sebagai Digital Exhibitor</a></h6>
                        </div>
                    </div>
                    
                </div>
            </div>
        </section>
        <!-- end section -->
